...
def start(camera, run, activation_flag):
    ...
    if s[1] == 1:  # this part wont run until background captured
        ...
        _,contours_1, hierarchy_1 = cv2.findContours(thresh1, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
        thresh2 = copy.deepcopy(thresh_2)
        _,contours_2, hierarchy_2 = cv2.findContours(thresh2, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
